...
Ближайшее действие: реализовать переопределение настроек через переменные окружения
...
- [x] создать минимальный `config.example.yaml` только с реально поддержанными настройками;
- [x] не включать секреты в пример;
- [ ] подключить загрузку и валидацию конфигурации к точке запуска приложения.
