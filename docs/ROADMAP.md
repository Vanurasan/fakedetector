...
Ближайшее действие: определить AnalysisResultSummary и семантику list_recent,
затем завершить ResultRepository / JsonFileResultRepository
...
- [ ] определить протокол `ResultRepository`;
  ...
  - [ ] определить `list_recent` после фиксации полей `AnalysisResultSummary` и
    контрактного признака сортировки;
- [ ] реализовать `JsonFileResultRepository`;
  - [x] реализовать файловый core для `save`, `get`, `exists`;
  - [ ] реализовать `list_recent` после устранения контрактной неоднозначности;
...
Неблокирующий остаток: `CONTRACTS.md`, раздел 15.1, использует
`AnalysisResultSummary` как возвращаемый тип `list_recent`, но не определяет
модель, её поля и признак сортировки. До контрактного решения `list_recent` не
реализуется и не типизируется через догадки, а repository interface целиком не
