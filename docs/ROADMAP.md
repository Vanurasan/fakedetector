...
Ближайшее действие: повторить независимый финальный аудит Stage 3
...
| 3 | Приём и первичная проверка файлов | IN_PROGRESS | Безопасно принятый или отклонённый файл |
...
Increment 3 является последним функциональным инкрементом Stage 3. После его
успешного завершения функциональные критерии считаются реализованными, но Stage 3
остаётся `IN_PROGRESS`, а ближайшим действием становится один отдельный final
audit Stage 3 в отдельной audit-ветке. Audit implementation и audit fixes не
входят в Increment 3.
