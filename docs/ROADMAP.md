...
Ближайшее действие: Stage 8 — проверить узкие исправления S8-A01–S8-A03 и принять решение о закрытии этапа
Критические блокеры: отсутствуют
Реализация программы: Этапы 1–7 завершены; Stage 8 Macro 1 и Macro 2 реализованы, независимый финальный аудит и узкие исправления S8-A01–S8-A03 завершены, ожидается проверка владельцем
...
| 8 | JSON, API и WebUI | IN_PROGRESS | Реализация завершена; S8-A01–S8-A03 исправлены и ожидают проверки владельцем |
...
`CLOSE_STAGE7_UNCHANGED`; Stage 7 формально закрыт. Stage 8 находится в
`IN_PROGRESS`: Macro 1 и Macro 2 реализованы, независимый финальный аудит завершён,
узкие исправления S8-A01–S8-A03 ожидают проверки владельцем.
...
Stage 7 имеет статус `DONE`. Следующее действие текущего Stage 8 — проверка
владельцем узких исправлений S8-A01–S8-A03 и решение о закрытии этапа.

---

# Этап 8. JSON, API и WebUI — IN_PROGRESS
...
- [x] Macro 2 — утверждённые асинхронный HTTP API и WebUI с HTTP Basic;
  ...
  узкие исправления S8-A01–S8-A03 внесены и ожидают проверки владельцем.
...
Общий критерий функциональной реализации достигнут, но Stage 8 сохраняет статус
`IN_PROGRESS`: независимый финальный аудит завершён, его findings S8-A01–S8-A03
исправлены и ожидают проверки владельцем. После этой проверки возможно отдельное
решение владельца о закрытии этапа; `DONE/CLOSED` самостоятельно не устанавливается.

Финальный целевой набор проверок Macro 2: `291 passed`. Полный набор проверок
проекта: `uv lock --check`, Ruff, mypy, pre-commit, проверка запуска CLI и импорта,
`git diff --check` прошли; полный pytest — `1671 passed, 3 skipped`, покрытие
`90%`. Проверка ресурсов пакета подтвердила включение шаблонов Jinja2 и CSS в
wheel.
