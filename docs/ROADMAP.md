...
```text
Общий статус: READY_TO_START
Текущий этап: Этап 1 — Каркас проекта и конфигурация
Статус этапа: NOT_STARTED
Ближайшее действие: создать Git-репозиторий и исполняемый каркас Python 3.12 через uv
Критические блокеры: отсутствуют
Реализация программы: не начата по имеющимся проектным данным
...
| 1 | Каркас проекта и конфигурация | NOT_STARTED | Запускаемое приложение |
| 2 | Доменные модели и репозитории | NOT_STARTED | Типизированные модели контрактов |
...
# Этап 1. Каркас проекта и конфигурация — NOT_STARTED
...
- [ ] добавить `.gitignore` для `.venv`, `.env`, `runtime`, моделей и кэшей;
- [ ] зафиксировать Python 3.12 в `.python-version`;
- [ ] инициализировать проект через `uv`;
- [ ] создать `pyproject.toml` и `uv.lock`;
- [ ] создать пакет приложения;
- [ ] создать каталоги `src`, `tests`, `templates`, `static`, `config`;
- [ ] создать пустую контролируемую структуру `runtime` без пользовательских данных в Git.

### Инструменты качества

- [ ] подключить Ruff;
- [ ] подключить mypy;
- [ ] подключить pytest и pytest-cov;
- [ ] подключить pre-commit после появления базовых команд;
- [ ] добавить единые команды запуска проверок.

### Конфигурация

- [ ] реализовать Pydantic-модели корневой конфигурации;
- [ ] запретить неизвестные поля;
- [ ] загрузить YAML;
- [ ] объединить YAML с переменными окружения;
- [ ] проверить обязательные секции;
- [ ] реализовать безопасную ошибку некорректной конфигурации;
- [ ] создать минимальный `config.example.yaml` только с реально поддержанными настройками;
- [ ] не включать секреты в пример.

### Запуск

- [ ] создать фабрику приложения FastAPI;
- [ ] создать Uvicorn-команду запуска;
- [ ] добавить служебный health endpoint без бизнес-логики;
- [ ] добавить стартовое журналирование без секретов.

## Обязательные тесты

- [ ] корректная конфигурация загружается;
- [ ] неизвестное поле отклоняется;
- [ ] отсутствующий обязательный параметр отклоняется;
- [ ] секрет читается из окружения, а не из Git;
- [ ] приложение создаётся в тесте;
- [ ] health endpoint возвращает успешный ответ;
- [ ] Ruff и mypy проходят на каркасе.
...
# Этап 2. Доменные модели и репозитории — NOT_STARTED
...
- [ ] реализовать enum из `CONTRACTS.md`;
- [ ] реализовать `SourceContext`;
- [ ] реализовать файловые дескрипторы;
- [ ] реализовать `ValidationResult`;
...
- [ ] реализовать итоговый `AnalysisResult`;
- [ ] зафиксировать `schema_version`.

### Репозитории

- [ ] определить протокол `ResultRepository`;
- [ ] реализовать `JsonFileResultRepository`;
- [ ] реализовать атомарную запись;
- [ ] реализовать безопасное чтение;
- [ ] обработать повреждённый JSON;
- [ ] исключить прямую запись результата другими модулями.

### Идентификаторы и время

- [ ] реализовать генератор непрозрачного `analysis_id`;
- [ ] реализовать единый UTC clock provider для тестируемости;
- [ ] исключить персональные данные из идентификатора.

## Обязательные тесты

- [ ] сериализация/десериализация каждой модели;
- [ ] неверные enum отклоняются;
- [ ] некорректная локализация отклоняется;
- [ ] probability без метода отклоняется;
- [ ] critical override без finding IDs отклоняется;
- [ ] правила терминальных статусов проверяются;
- [ ] результат записывается атомарно;
- [ ] путь строится только по `analysis_id`.
...
# Этап 3. Приём и первичная проверка файлов — NOT_STARTED

## Цель

Реализовать единый безопасный путь получения файла и его отклонения до ресурсоёмкого анализа.

## Обязательные задачи

### Единый сервис приёма

- [ ] определить прикладной сервис приёма файла;
- [ ] WebUI и API должны использовать один сервис;
- [ ] создать `analysis_id` и `SourceContext`;
- [ ] ограничить чтение размера до полной загрузки, насколько позволяет слой приёма;
- [ ] исключить использование оригинального имени в системном пути.
...
- [ ] анализаторы не запускаются;
- [ ] риск не формируется;
- [ ] причина сохраняется машинным кодом;
- [ ] пользователь получает безопасное сообщение;
- [ ] временные данные очищаются.

## Обязательные тесты

- [ ] допустимые image/audio/video проходят;
- [ ] архив отклоняется;
- [ ] офисный документ отклоняется;
- [ ] переименованный файл с неверной сигнатурой отклоняется;
- [ ] файл сверх лимита отклоняется;
- [ ] повреждённый файл отклоняется;
- [ ] SHA-256 стабилен;
- [ ] path traversal в имени не влияет на путь;
- [ ] отклонение не создаёт риск `low` или `high`.

## Критерий завершения

Сервис однозначно возвращает либо проверенный файловый дескриптор, либо терминальный результат `rejected`, не запуская анализаторы.
...
Создать управляемую задачу анализа с внутренней очередью, изолированным workspace и гарантированной очисткой.

## Обязательные задачи

### Workspace

- [ ] создать `runtime/temp/<analysis_id>`;
- [ ] размещать только необходимые рабочие данные;
- [ ] ограничить доступ к каталогу;
- [ ] реализовать реестр промежуточных артефактов;
- [ ] исключить прямое включение пользовательских строк в путь.
...
- [ ] выполнять очистку в `finally`;
- [ ] удалять исходные и промежуточные файлы;
- [ ] повторять удаление по настройке;
- [ ] использовать quarantine только при сбое;
...
- [ ] разные analysis_id получают разные каталоги;
- [ ] очистка выполняется после успеха;
- [ ] очистка выполняется после исключения;
...
Тестовая задача проходит регистрацию, очередь, маршрутизацию и очистку, а её состояние можно восстановить по доменной модели.
...
- [ ] безопасно открыть изображение;
- [ ] извлечь технические параметры;
...
- [ ] получить параметры через FFmpeg/ffprobe безопасным вызовом;
...
На дату фиксации нет блокеров для этапов 1–5.
