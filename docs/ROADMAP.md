...
Ближайшее действие: реализовать минимальную YAML-конфигурацию, Pydantic-валидацию и переопределение настроек через переменные окружения
...
| 1 | Каркас проекта и конфигурация | IN_PROGRESS | Запускаемое приложение |
...
# Этап 1. Каркас проекта и конфигурация — IN_PROGRESS
...
- [ ] создать каталог `config` для примера конфигурации;
- [ ] обеспечить автоматическое создание необходимых каталогов `runtime` при запуске приложения;
- [ ] не хранить содержимое `runtime` в Git.
...
- [ ] настройка может быть переопределена переменной окружения;
- [ ] секретные поля, когда они появятся, не включаются в пример конфигурации и Git.
