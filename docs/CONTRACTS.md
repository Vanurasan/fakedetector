...
- общий интерфейс анализатора и модели его результата;
- контракты требований и возможностей предобработки;
...
- конкретные forensic-алгоритмы, применимость метода и production-пороги — это область `METHODS.md`;
- происхождение и лицензии внешних источников — это область `REFERENCES.md`;
- процесс работы агента — это область `../AGENTS.md`;
...
### 0.2. Полномочия по областям

Общее ранжирование документов не применяется; карта областей — `PROJECT.md`
§0.2. `CONTRACTS.md` владеет интерфейсами, данными, схемами, валидацией,
сериализацией, ошибками и failure semantics, общими контрактами analyzer,
result/finding и preprocessing requirement/capability. Они не переносятся
в `METHODS.md`: тот задаёт конкретную методологию внутри этих контрактов.

Противоречие нормативных областей требует owner gate. Код и тесты служат
свидетельствами реализации; расхождение с документацией — drift, а не молчаливое
изменение контракта. Архивные файлы и история в `CHANGELOG.md` не переопределяют
текущие решения. Это уточнение владения не меняет продуктовые контракты.
...
`hide_internal_error_details` допускает только `true` (значение по умолчанию).
`false` отклоняется при валидации: раскрытие внутренних исключений через API
или WebUI не поддерживается.
...
В MVP интеграции отсутствуют. Поле `enabled` допускает только `false`
(значение по умолчанию); `true` отклоняется при валидации.

Оба фиксированных поля сохраняются в схеме `1.0`. Генерируемая JSON Schema
описывает их через `const`; имена и значения по умолчанию не изменены.
Канонический JSON и `config_snapshot_id` ранее поддерживаемой конфигурации
с этими значениями остаются прежними; алгоритм снимка не меняется.
