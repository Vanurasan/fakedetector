...
- **[Stage 6/Audit status] Блокирующие замечания независимого аудита устранены.**
  Новый полный барьер качества прошёл: `1401 passed, 2 skipped`, совокупное
  покрытие `89.74%`, покрытие операторов `92.19%`, покрытие ветвей `79.79%`.
  Этап остаётся `IN_PROGRESS`: реализация завершена, блокирующие замечания
  устранены, повторный независимый аудит закрытия ещё не выполнен; `PASS`
  повторного аудита не утверждается.
- **[Stage 6/S6-LIM-001] Принята неоднозначность прерывания до подтверждения
  ...
  остаток `S6-AUD-002` имеет статус `ACCEPTED_RUNTIME_LIMITATION` и не блокирует
  закрытие Stage 6. После неудачного старта планировщик уже отклоняет новые
...
- **[Stage 6/S6-AUD-002] Закрыта утечка потока при прерывании частичного старта
  планировщика.** До распространения ошибки планировщик переходит в завершающее
  состояние и явно ожидает все потоки, запуск которых наблюдаем через публичное
  состояние `threading`; `Thread.ident` используется как лучшее доступное
  наблюдение, но его отсутствие не доказывает, что нативный поток не создан.
  Обычное `Exception` сохраняет `SchedulerStateError`, исходный
  `KeyboardInterrupt`/`SystemExit` поднимается повторно. Остаточное pre-ack окно
  отдельно принято как `S6-LIM-001`.
